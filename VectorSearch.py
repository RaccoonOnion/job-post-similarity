...
class VectorSearch:
    ...
    def __init__(self, dimension, index_description='IndexFlatL2'):
        ...
        print(f"Initializing VectorSearch with dimension={dimension} and index='{index_description}'")
        try:
            # --- MODIFICATION START ---
            ...
            if index_description == 'IndexFlatL2':
                ...
                print(f"Using index_factory for description: '{index_description}'")
            ...
            if self.index.d != self.dimension:
                 ...
                 raise RuntimeError(f"Faiss index created with incorrect dimension: {self.index.d} != {self.dimension}")
        except Exception as e:
            # Add more context to the error
            raise RuntimeError(f"Failed to create Faiss index with description '{index_description}'. Original error: {e}")
    ...
    def train(self, embeddings):
        ...
        if embeddings.ndim != 2 or embeddings.shape[1] != self.dimension:
            raise ValueError(f"Embeddings must be a 2D array with shape (*, {self.dimension}).")
        ...
        if not self.index.is_trained:
            ...
            print(f"Index type requires training. Training with {embeddings.shape[0]} vectors...")
            try:
    ...
    def add(self, embeddings, ids):
        ...
        try:
            ...
            print(f"Vectors added successfully. Index now contains {self.index.ntotal} vectors.")
            print(f"ID map now contains {len(self.id_map)} entries.")
            # Sanity check
            if self.index.ntotal != len(self.id_map):
                 print(f"Warning: Mismatch between index total ({self.index.ntotal}) and id_map length ({len(self.id_map)}). Check logic.")
    ...
    def search(self, query_embeddings, k):
        ...
        if query_embeddings.ndim != 2 or query_embeddings.shape[1] != self.dimension:
            raise ValueError(f"Query embeddings must be a 2D array with shape (*, {self.dimension}).")
        ...
        if self.index.ntotal == 0:
            ...
            empty_distances = np.full((num_queries, k), np.inf, dtype=np.float32)
            empty_ids = [[None for _ in range(k)] for _ in range(num_queries)] # Use None for clarity
        ...
        if effective_k < k:
            print(f"Warning: Requested k={k} neighbors, but index only contains {self.index.ntotal} vectors. Searching for {effective_k} neighbors.")
        ...
        print(f"Searching for {effective_k} nearest neighbors for {query_embeddings.shape[0]} queries...")
        try:
            distances, indices = self.index.search(query_embeddings, effective_k)
            ...
            for query_indices in indices:
                ...
                for idx in query_indices:
                    if idx == -1: # Faiss uses -1 if fewer than k neighbors are found (or on error)
                        query_neighbor_ids.append(None)
                    elif 0 <= idx < len(self.id_map):
                        query_neighbor_ids.append(self.id_map[idx])
                    else:
                        print(f"Warning: Found invalid index {idx} during search result mapping.")
            ...
            if distances.shape[1] < k:
                 padded_distances = np.full((distances.shape[0], k), np.inf, dtype=np.float32)
    ...
    def save(self, index_path, id_map_path):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to save Faiss index to {index_path}: {e}")
    ...
    def load(self, index_path, id_map_path):
        ...
        try:
            ...
            print(f"Index loaded successfully. Dimension={self.dimension}, NumVectors={self.index.ntotal}")
        except Exception as e:
        ...
        try:
            ...
            print(f"ID map loaded successfully. Map contains {len(self.id_map)} entries.")
        except Exception as e:
            raise RuntimeError(f"Failed to load ID map from {id_map_path}: {e}")

        # Sanity check
        if self.index.ntotal != len(self.id_map):
            # Allow loading even if mismatch, but print strong warning
            print(f"CRITICAL WARNING: Loaded index total ({self.index.ntotal}) does not match loaded id_map length ({len(self.id_map)}). Search results may be incorrect!")
...
if __name__ == '__main__':
    ...
    try:
        ...
        vector_searcher = VectorSearch(dimension=D) # Uses IndexFlatL2 by default
        ...
        distances, neighbor_ids = vector_searcher.search(dummy_queries, k=K_NEIGHBORS)
        ...
        if load_success:
            ...
            distances_loaded, neighbor_ids_loaded = loaded_searcher.search(dummy_queries, k=K_NEIGHBORS)
            ...
            original_neighbors_str = [[str(nid) for nid in sublist] for sublist in neighbor_ids]
            loaded_neighbors_str = [[str(nid) for nid in sublist] for sublist in neighbor_ids_loaded]
